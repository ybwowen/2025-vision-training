...
int main() {
    ...
    // 初始化相机参数
    cameraMatrix = (cv::Mat_<float>(3,3) << 400, 0, 190, // 相机内参矩阵
                                                        0, 400, 160, 
                                                        0, 0, 1); 
    ...
    cv::Mat invert_rot;
    cv::invert(rotationMatrix,invert_rot);
    ...
    // 创建图像并设置大小（根据需要调整）
    cv::Mat image(1080, 1080, CV_8UC3, cv::Scalar(255,255, 255));
    ...
    for (const cv::Point3f& point3D : worldPoints) 
    {
        ...
        cv::Mat pointCam=invert_rot*point3D_cv-translationVector;
        std::cout<<pointCam<<std::endl;
        ...
        // 绘制点
        std::cout<<point2D<<std::endl;
        cv::circle(image, cv::Point(cvRound(point2D.at<float>(0, 0)), cvRound(point2D.at<float>(1, 0))), 2, cv::Scalar(0, 0, 255), -1);
    }

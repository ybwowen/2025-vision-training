...
public:
    DivisionServer() : Node("division_server")
    {
        service_ = this->create_service<DivideInt>(
            "divide_two_ints", 
            std::bind(&DivisionServer::handle_service, this, std::placeholders::_1, std::placeholders::_2)
        );
        RCLCPP_INFO(this->get_logger(), "Service 'divide_two_ints' ready to receive requests.");
    }
